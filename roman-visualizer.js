...
let phaseTimer = 0;
let regionTransitionTimer = 0;
let activeRegionIndex = 0;

// Overworld Map Matrices
let biomeGrid = [];
let riverTiles = [];
let townTiles = [];
let campCenter = { r: 12, c: 20 }; // Dynamically designated location to build next
...
function generateWorldMap() {
    ...
    // 1. Roll randomized regional biome divides across quadrants
    activeRegionIndex = Math.floor(Math.random() * REGIONS.length);
    for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
            // Slight mathematical drift to blend border boundaries organically
    ...
    // 2. Carve a winding River across the map matrix
    let startC = Math.floor(Math.random() * (COLS - 10)) + 5;
    for (let r = 0; r < ROWS; r++) {
        // Sine wave offset to simulate realistic organic river bends
        ...
        if (targetC >= 0 && targetC < COLS) {
            ...
            riverTiles.push({ r, c: targetC + 1 }); // Two tiles wide for depth
        }
    }

    // 3. Sprout independent provincial towns away from the central river channel
    ...
    // 4. Select a clear, safe flatland space to serve as our encampment coordinate
    ...
    while (!possibleCampFound) {
        ...
        // Check validation boundaries to prevent overlapping towns or water bodies
...
function initCampBlueprints() {
    ...
    // 1. Plan Outpost Borders (Ramparts)
    ...
    // 3. Structural Province Flagship Node
    ...
    // 4. Randomized Soldier Quarter Tents
    ...
    // Unpredictable Fisher-Yates task shuffle
...
function spawnCohort() {
    legionCohort = [];
    // Spawns column off-screen left, ready to begin routing path across terrain
    for(let i = 0; i < COHORT_SIZE; i++) {
        legionCohort.push({
            x: -i * 15 - 30,
            y: Math.floor(ROWS / 2) * GRID_SIZE + 10,
            color: '#e74c3c',
            history: [] // Tracks path history for smooth single-file snaking
        });
    }
}
...
function update() {
    ...
    if (currentPhase === PHASES.WORLD_MARCH) {
        ...
        // Once the vanguard captain arrives at the destination clearance, transition to build phase
    ...
    // --- PHASE 3: CAMP LIFE & AUTONOMOUS PATROLS ---
        ...
        patrols.forEach(p => {
            ...
            if (p.targetX === null || p.targetY === null) {
                // Sentries patrol dynamically anywhere inside the extended boundary walls
        ...
        // Ambient cooking fires smoke engine
        if(Math.random() < 0.2) {
            particles.push({
        ...
        if (phaseTimer > CAMP_LIVE_DURATION) { 
            ...
            patrols = []; 
            spawnCohort();
            // Assemble cohort in the center of camp, ready to pack out
            legionCohort.forEach(u => { u.x = campCenter.c * GRID_SIZE + 10; u.y = campCenter.r * GRID_SIZE + 10; });
        }
    } 
    // --- PHASE 4: RANDOMIZED DECONSTRUCTION ---
        ...
        legionCohort.forEach(unit => {
            // March off toward the right side of the overworld map canvas
...
function draw() {
    ...
    // 1. Draw Render Overworld Procedural Biome Grids
    ...
    // 2. Draw Procedural Winding Blue Rivers
    ctx.fillStyle = '#2980b9';
    riverTiles.forEach(t => {
        ctx.fillRect(t.c * GRID_SIZE, t.r * GRID_SIZE, GRID_SIZE, GRID_SIZE);
    });

    // 3. Draw Local Towns (Earthy Brown Squares with Cross Hatching thatched roofs)
    ...
    // 4. Render Active Outpost Fortifications
    for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
            const cell = grid[r][c];
            if (!cell) continue;
    ...
    // Render Single-File Marching Cohort
    if (currentPhase === PHASES.WORLD_MARCH || currentPhase === PHASES.DEMOLISH) {
        legionCohort.forEach((unit, idx) => {
            ctx.fillStyle = idx === 0 ? '#f1c40f' : unit.color; // Captain wears golden helmet armor
            ctx.fillRect(unit.x - 3, unit.y - 3, 6, 6);
            
            // Draw standard wooden unit shields (Scuta) flanking sides
